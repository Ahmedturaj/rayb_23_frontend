...
export default function InboxComponent({ config }: InboxComponentProps) {
    ...
    const { data: chats = [], refetch } = useQuery({
        ...
        enabled: !!myUserId && (config.additionalData ? !!config.additionalData : true),
    })
    ...
    useEffect(() => {
        ...
        socket.on("message", (msg: Message) => {
            console.log("New message received:", {
                ...
                matches: msg?.chat === currentChatRef.current,
                msgChat: msg?.chat,
            ...
            if (msg?.chat === currentChatRef.current) {
                setLiveMessages((prev) => {
                    console.log("Processing message, current messages:", prev.length)

                    // More lenient deduplication - only check for exact ID matches or tempId matches
                    const exists = prev.some((m) => {
                        const idMatch = m._id === msg._id && msg._id !== `temp-${Date.now()}`
                        const tempIdMatch = m.tempId && msg.tempId && m.tempId === msg.tempId
                        return idMatch || tempIdMatch
                    })

                    if (exists) {
                        console.log("Message exists, replacing optimistic message")
    ...
    const sendMutation = useMutation({
        mutationFn: (formData: FormData) => sendMessage({ data: formData }).then((res) => res.data),
        onSuccess: () => {
            setNewMessage("")
            queryClient.invalidateQueries({ queryKey: config.queryKey })
        },
    ...
    const handleSend = () => {
        ...
        const tempId = `temp-${myUserId}-${Date.now()}-${Math.random()}`
        const payload = {
                        ...
                        {chats?.length > 0 ? (
                            ...
                            chats.map((chat: any) => (
                                <div
                                    ...
                                    className={`p-4 rounded-xl cursor-pointer hover:bg-[#F7F8F8] transition-colors ${config.getChatId(selectedChat) === config.getChatId(chat) ? "bg-[#F7F8F8]" : ""
                                        }`}
                                    onClick={() => handleChatSelect(chat)}
                                >
                                    <div className="flex items-center space-x-3">
                                        <Avatar className="h-12 w-12">
                                            <AvatarImage src={config.getChatImage(chat) || "/placeholder.svg"} />
                                        ...
                                        <div className="flex-1">
                                            <div className="flex items-center justify-between">
                                                <p className="text-sm font-medium text-gray-900 truncate">{config.getChatName(chat)}</p>
                            ...
                            <div className="flex items-center justify-between">
                                <div className="flex items-center space-x-3">
                                    ...
                                    <div className="flex items-center gap-5">
                                        ...
                                        <p className="text-sm text-gray-500 hidden sm:block">{config.getChatEmail(selectedChat)}</p>
                                    </div>
                                </div>
                                <Avatar className="h-12 w-12 mt-1">
                                    <AvatarImage src={config.getChatImage(selectedChat) || "/placeholder.svg"} />
                                            ...
                                            <div className="flex items-start space-x-2 max-w-xs lg:max-w-md">
                                                {!isMyMessage && (
                                                    <Avatar className="h-8 w-8 mt-1">
                                                        <AvatarImage src={senderImage || config.getChatImage(selectedChat) || "/placeholder.svg"} />
                                                        <AvatarFallback className="bg-gray-200 text-gray-600 text-xs">
                                                            {getInitials(senderName || config.getChatName(selectedChat))}
                                                        </AvatarFallback>
                                                    </Avatar>
                                                )}
                                                <div
