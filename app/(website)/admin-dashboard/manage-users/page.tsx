...
const fetchUsers = async ({
  ...
  const params = new URLSearchParams({
    ...(userType !== 'all' && { userType: userType === 'businessOwner' ? 'businessOwner' : userType }),
    ...(sortBy !== 'latest' && { sortBy }),
    ...(time !== 'all' && { time })
  });
  
  const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/user?${params}`);
...
const deactivateUser = async ({reason, token }: { userId: string; reason: string; token?: string }) => {
  ...
  const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/user/deactive-account`, {
    method: 'PUT',
    headers,
...
export default function ManageUsersPage() {
  ...
  const confirmDeactivate = () => {
    if (selectedUserId) {
      deactivateMutation.mutate({ userId: selectedUserId, reason: deleteReason });
    }
  };
          ...
          <tbody className="[&_tr:last-child]:border-0">
            {isLoading ? (
              Array(8).fill(0).map((_, index) => (
                <tr key={index} className="border-b transition-colors hover:bg-muted/50 data-[state=selected]:bg-muted even:bg-muted/50">
             ...
             // eslint-disable-next-line 
              users?.map((user: { _id: Key | null | undefined; imageLink: any; name: string | number | bigint | boolean | ReactElement<any, string | JSXElementConstructor<any>> | Iterable<ReactNode> | Promise<AwaitedReactNode> | null | undefined; email: string | number | bigint | boolean | ReactElement<any, string | JSXElementConstructor<any>> | Iterable<ReactNode> | ReactPortal | Promise<AwaitedReactNode> | null | undefined; phone: any; createdAt: string | number | Date; userType: string; }) => (
                ...
                >
                  ...
                  <td className="p-4 align-middle [&:has([role=checkbox])]:pr-0 text-base font-medium text-[#252525]">
                    {user.userType === 'businessOwner' ? 'Business Owner' : 
          ...
          {isUserDetailsLoading ? (
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <p>{userDetails.userType === 'businessOwner' ? 'Business Owner' : 'Customer'}</p>
              </div>
              <div>
                <Label>Bio</Label>
