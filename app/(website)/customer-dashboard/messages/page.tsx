...
export default function InboxPage() {
    ...
    const { data: messages = [] } = useQuery({
        ...
        queryFn: () => (selectedChat ? getMessages(selectedChat._id).then((res) => res.data) : []),
                    ...
                    const messageExists = prev.some(
                        (existingMsg) =>
                            ...
                            (existingMsg.message === msg.message &&
                                ...
                                Math.abs(
                                    new Date(existingMsg.createdAt || existingMsg.date).getTime() -
                                ...
                                >
                                    <div className="flex items-center space-x-3">
                                        <Avatar className="h-12 w-12">
                                            <AvatarImage
                                                src={chat?.bussinessId?.businessInfo?.image[0]}
                                            />
                        ...
                        <ScrollArea className="flex-1 p-4" ref={messagesScrollAreaRef}>
                            <div className="space-y-4">
                                ...
                                {liveMessages?.map((msg: any, index: number) => {
                                    const isMyMessage = msg?.senderId?._id === myUserId || msg?.senderId === myUserId
                                    return (
                                        <div
